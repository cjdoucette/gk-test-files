...
if [ $# -ne 2 ]; then
  echo "usage: ./run.sh exp_name traffic_rate"
  echo "e.g.: ./run.sh nogk {100mibps,500mibps,1-9gibps}"
  exit
fi
...
./ec2_send_command.sh client2 "sudo pkill sendRaw"
...
./ec2_send_file.sh gk1_server "tests/nogk/config/add.lua" "gatekeeper/lua/examples/add.lua"
...
./ec2_send_command.sh client "nohup bash -c 'cd raw-packets; sudo make; sudo ./sendRaw ${EXP_RATE}' &>/dev/null &"
sleep 1
./ec2_send_command.sh client2 "nohup bash -c 'cd raw-packets; sudo make; sudo ./sendRaw ${EXP_RATE}' &>/dev/null &"
